...
namespace Property
{
    public partial class RealEstateNews : System.Web.UI.Page
    {
        ...
        public  void mortgagedaily()
        {
            ...
            dtable.Merge(getxmldata());
            dtable.AcceptChanges();
            //dtable.Merge(getcnbcNews());
            //dtable.AcceptChanges();
            DataView _dv = new DataView(dtable);
            _dv.Sort = "pubDate DESC";
            grvRSS.DataSource = dtable;           
            grvRSS.DataBind();
           // return dtable;
        }

        public DataTable getcnbcNews()
        {
            ...
            dtable.Columns.Add(new DataColumn("guid"));
            dtable.Columns.Add(new DataColumn("pubDate"));
            WebRequest WebReq = WebRequest.Create("https://www.cnbc.com/id/10000115/device/rss/rss.html");
            WebResponse webRes = WebReq.GetResponse();
            Stream rssStream = webRes.GetResponseStream();
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(rssStream);
